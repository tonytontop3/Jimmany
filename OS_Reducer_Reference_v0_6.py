...
EventType = Literal[
    ...
    "DM_REVIEWED",
    "DM_RESOLVED",
    "DM_FAILURE_MODE_DETECTED",
    "DM_CALCIFIED_FLAGGED",
    "DM_EMERGENCY_OVERRIDE",
    "SD_METRICS_UPDATED",
    "SD_EVALUATION",
    "SD_CLASS0_INVOCATION",
]# -------------------------
...
class DMWindow:
    ...
    episodes: int = 0
    u_sum: float = 0.0
    overconfidence_hits: int = 0
    calcified_hits: int = 0
    action_regret_hits: int = 0
    last_updated: Optional[str] = None

    def snapshot(self) -> Dict[str, Any]:
        u_avg = (self.u_sum / self.episodes) if self.episodes else 3.0
        return {
            "window_size": self.window_size,
            "episodes": self.episodes,
            "u_avg": max(1.0, min(5.0, u_avg)),
            "overconfidence_rate": (self.overconfidence_hits / self.episodes) if self.episodes else 0.0,
            "calcified_rate": (self.calcified_hits / self.episodes) if self.episodes else 0.0,
            "action_regret_rate": (self.action_regret_hits / self.episodes) if self.episodes else 0.0,
...
class DoubtModule:
    dm_version: str = "0.1"
    status: str = "CLEAR"  # CLEAR|ACTIVE|CALCIFIED|EMERGENCY_OVERRIDE
    active_case_id: Optional[str] = None
    u_floor: float = 3.0
    triggers_active: List[str] = field(default_factory=list)
    calcified_flags: List[str] = field(default_factory=list)
    failure_modes_seen: List[str] = field(default_factory=list)

    # DCL: ring buffer of dict entries (validated by schema elsewhere)
    dcl: List[Dict[str, Any]] = field(default_factory=list)
    dcl_maxlen: int = 200

    # rolling windows (fast=10, main=30) used by ALSC/SWS and Drift Audit
    rolling_fast10: DMWindow = field(default_factory=lambda: DMWindow(window_size=10))
    rolling_main30: DMWindow = field(default_factory=lambda: DMWindow(window_size=30))

    drift_risk: float = 0.0
    last_review_ts: Optional[str] = None

    def append_dcl(self, entry: Dict[str, Any]) -> None:
        self.dcl.append(entry)
        if len(self.dcl) > self.dcl_maxlen:
            # Evict oldest
            self.dcl = self.dcl[-self.dcl_maxlen:]

# -------------------------
# Stubbornness Differential (SD) — minimal state module
# -------------------------

@dataclass
class SDWindow:
    gsr: float = 0.0   # gate-stall ratio (or similar)
    rd: float = 0.0    # refusal density
    rgl: int = 0       # repeat-gate length
    pd: float = 0.0    # progress delta
    rt: int = 0        # recovery time


@dataclass
class SDModule:
    last_gate_outcome: str = ""
    repeat_gate_streak: int = 0
    gate_stall_count: int = 0
    refuse_count: int = 0
    steps: int = 0
    progress_delta_last: float = 0.0
    recovery_time_current: int = 0
    rolling_w10: SDWindow = field(default_factory=SDWindow)
    rolling_w30: SDWindow = field(default_factory=SDWindow)
    last_label: str = ""
    last_reason: str = ""
...
class State:
    ...
    # Doubt Module (DM) + Doubt Case Ledger (DCL)
    doubt: DoubtModule = field(default_factory=DoubtModule)

    # Stubbornness Differential (SD)
    sd: SDModule = field(default_factory=SDModule)

    # Tie-breaker history (deadlock resolution)
    tiebreak_count: int = 0
    last_tiebreak: Optional[dict] = None
...
def apply_event(state: State, event: EventEnvelope) -> State:
    ...
    # Governance unlock events (v0.5)
    if etype == "DRIFT_REVIEW_UNLOCK_REQUESTED":
        _gov_unlock_requested(new_state, p.get("governance", {}))
        return new_state
    if etype == "DRIFT_REVIEW_UNLOCK_GRANTED":
        _gov_unlock_granted(new_state, p.get("governance", {}))
        return new_state
    if etype == "DRIFT_REVIEW_LOCKS_CLEARED":
        _gov_clear_locks(new_state, p.get("governance", {}))
        return new_state
    if etype == "CANON_FREEZE_LIFTED":
        _gov_canon_freeze_lifted(new_state, p.get("governance", {}))
        return new_state
    if etype == "AUTHORITY_GROWTH_UNLOCKED":
        _gov_authority_growth_unlocked(new_state, p.get("governance", {}))
        return new_state
    if etype == "D3_ESCALATION_RESTORED":
        _gov_d3_restored(new_state, p.get("governance", {}))
        return new_state


    # Doubt Module (DM v0.1) + Doubt Case Ledger (DCL)
    if etype == "DM_TRIGGERED":
        new_state.doubt.status = "ACTIVE"
        new_state.doubt.active_case_id = p.get("case_id")
        new_state.doubt.u_floor = float(p.get("u_floor", new_state.doubt.u_floor))
        trig = p.get("trigger")
        if trig:
            if trig not in new_state.doubt.triggers_active:
                new_state.doubt.triggers_active.append(str(trig))
        claim = p.get("claim")
        if claim:
            # Store claim as a minimal DCL seed entry
            entry = {
                "case_id": p.get("case_id", "case_unknown"),
                "ts": event.ts,
                "trigger": str(trig or "unknown"),
                "claim": str(claim),
                "u": float(p.get("u", max(new_state.doubt.u_floor, new_state.uncertainty_u))),
                "fiv": str(p.get("fiv", "INFERENCE")),
                "decision_class": str(p.get("decision_class", new_state.decision_class.value)),
                "recommended_route": str(p.get("recommended_route", "PAUSE")),
                "notes": str(p.get("notes", "")),
                "status": "OPEN",
            }
            new_state.doubt.append_dcl(entry)
        return new_state

    if etype == "DCL_APPENDED":
        entry = p.get("entry")
        if isinstance(entry, dict):
            new_state.doubt.append_dcl(entry)
        return new_state

    if etype == "DM_CALCIFIED_FLAGGED":
        new_state.doubt.status = "CALCIFIED"
        flags = p.get("flags", [])
        if isinstance(flags, list):
            for f in flags:
                if f not in new_state.doubt.calcified_flags:
                    new_state.doubt.calcified_flags.append(str(f))
        return new_state

    if etype == "DM_REVIEWED":
        new_state.doubt.last_review_ts = event.ts
        # record reviewer note as evidence without control transfer
        ...
        if disp == "RESOLVE":
            new_state.doubt.status = "CLEAR"
            new_state.doubt.active_case_id = None
            new_state.doubt.triggers_active = []
            new_state.doubt.calcified_flags = []
        elif disp in {"RAISE_U_FLOOR"}:
            new_state.doubt.u_floor = max(new_state.doubt.u_floor, float(p.get("u_floor", new_state.doubt.u_floor + 0.5)))
        return new_state

    if etype == "DM_RESOLVED":
        new_state.doubt.status = "CLEAR"
        new_state.doubt.active_case_id = None
        new_state.doubt.triggers_active = []
        new_state.doubt.calcified_flags = []
    ...
    if etype == "DM_EMERGENCY_OVERRIDE":
        new_state.doubt.status = "EMERGENCY_OVERRIDE"
        new_state.doubt.active_case_id = p.get("case_id", new_state.doubt.active_case_id)
        # Downshift authority if requested
        ds = p.get("authority_downshift")
        if ds == "D3_TO_D2" and new_state.decision_class == DecisionClass.D3:
            new_state.decision_class = DecisionClass.D2
        if ds == "D2_TO_D1" and new_state.decision_class == DecisionClass.D2:
            new_state.decision_class = DecisionClass.D1
        return new_state

    if etype == "DM_FAILURE_MODE_DETECTED":
        mode = p.get("mode")
        if mode:
            if mode not in new_state.doubt.failure_modes_seen:
                new_state.doubt.failure_modes_seen.append(str(mode))
        return new_state




    # Stubbornness Differential (SD) events
    if etype == "SD_METRICS_UPDATED":
        sd = new_state.sd
        sd.last_gate_outcome = str(p.get("gate_outcome", sd.last_gate_outcome))
        sd.steps = int(p.get("step", sd.steps))
        # rolling window metrics (w10/w30 may be updated externally; we store point values)
        for win_key, win_attr in [("w10","rolling_w10"), ("w30","rolling_w30")]:
            w = getattr(sd, win_attr)
            # accept either names from payload or nested dicts if present later
            w.gsr = float(p.get("gsr", w.gsr))
            w.rd  = float(p.get("rd",  w.rd))
            w.rgl = int(p.get("rgl", w.rgl))
            w.pd  = float(p.get("pd",  w.pd))
            w.rt  = int(p.get("rt",  w.rt))
        return new_state

    if etype == "SD_EVALUATION":
        new_state.sd.last_label = str(p.get("label", new_state.sd.last_label))
        new_state.sd.last_reason = str(p.get("reason", new_state.sd.last_reason))
        return new_state

    if etype == "SD_CLASS0_INVOCATION":
        # record a class-0 safeguard invocation marker
        new_state.flags.high_risk = True
        new_state.last_gate = GateDecision.PAUSE
        new_state.last_action_summary = "sd_class0_invocation_pause"
        return new_state
    ...
    elif etype == "TIEBREAK_APPLIED":
        # Deterministic deadlock resolution: select controlling mode and log rationale.
        sel = (p.get("selected_mode") or "AEGIS").upper()
        try:
            new_state.mode = Mode[sel]
        except Exception:
            new_state.mode = Mode.AEGIS
            sel = "AEGIS"
        # Persist tiebreak info in state.meta (lightweight)
        if not hasattr(new_state, "tiebreak"):
            # Back-compat: attach dynamically if older State lacks field
            pass
        # Store in Flags or last_action_summary for audit
        new_state.tiebreak_count = getattr(new_state, "tiebreak_count", 0) + 1
        new_state.last_tiebreak = {
            "selected_mode": sel,
            "priority_order": p.get("priority_order", []),
            "reason": p.get("reason", ""),
            "conflict_tags": p.get("conflict_tags", []),
            "rule_id": p.get("rule_id", "OS-TB-001"),
        }
        new_state.last_action_summary = canonical_json({
            "tiebreak": {
                "selected_mode": sel,
                "priority_order": p.get("priority_order", []),
                "reason": p.get("reason", ""),
...
def copy_state(state: State) -> State:
    ...
    return State(
        ...
        doubt=DoubtModule(
            dm_version=state.doubt.dm_version,
            status=state.doubt.status,
            u_floor=state.doubt.u_floor,
            triggers_active=list(state.doubt.triggers_active),
            active_case_id=state.doubt.active_case_id,
            cases=list(state.doubt.cases),
        ),
        sd=SDModule(
            last_gate_outcome=state.sd.last_gate_outcome,
            repeat_gate_streak=state.sd.repeat_gate_streak,
            gate_stall_count=state.sd.gate_stall_count,
            refuse_count=state.sd.refuse_count,
            steps=state.sd.steps,
            progress_delta_last=state.sd.progress_delta_last,
            recovery_time_current=state.sd.recovery_time_current,
            rolling_w10=SDWindow(**state.sd.rolling_w10.__dict__),
            rolling_w30=SDWindow(**state.sd.rolling_w30.__dict__),
            last_label=state.sd.last_label,
            last_reason=state.sd.last_reason,
        ),
        tiebreak_count=state.tiebreak_count,
        last_tiebreak=state.last_tiebreak,
...
# --- Governance unlock wiring (v0.5) ---
def _ensure_gov_log(new_state):
    if not hasattr(new_state, "meta") or new_state.meta is None:
        return None
    gl = getattr(new_state.meta, "governance_log", None)
    if gl is None:
        new_state.meta.governance_log = []
        gl = new_state.meta.governance_log
    return gl

def _gov_record(new_state, etype: str, payload: dict):
    gl = _ensure_gov_log(new_state)
    if gl is None:
        return
    gl.append({
        "etype": etype,
        ...
        "min_approvals": payload.get("min_approvals"),
        "locks": payload.get("locks") or {}
    })

def _gov_unlock_requested(new_state, payload: dict):
    dr = _ensure_drift_review_obj(new_state)
    if dr is None:
        return
    # does not change locks; only records intent
    _gov_record(new_state, "DRIFT_REVIEW_UNLOCK_REQUESTED", payload)

def _gov_unlock_granted(new_state, payload: dict):
    dr = _ensure_drift_review_obj(new_state)
    if dr is None:
        return
    _gov_record(new_state, "DRIFT_REVIEW_UNLOCK_GRANTED", payload)

def _gov_clear_locks(new_state, payload: dict):
    dr = _ensure_drift_review_obj(new_state)
    if dr is None:
        return
    locks = (payload.get("locks") or {})
    # default: clear all three if unspecified
    if locks == {}:
        locks = {"authority_growth": True, "canon_freeze": True, "d3_escalation": True}
    if locks.get("authority_growth"):
        dr["authority_growth_locked"] = False
    if locks.get("canon_freeze"):
        dr["canon_modification_frozen"] = False
    if locks.get("d3_escalation"):
        dr["d3_escalation_forbidden"] = False
    _gov_record(new_state, "DRIFT_REVIEW_LOCKS_CLEARED", payload)

def _gov_canon_freeze_lifted(new_state, payload: dict):
    dr = _ensure_drift_review_obj(new_state)
    if dr is None:
        return
    dr["canon_modification_frozen"] = False
    _gov_record(new_state, "CANON_FREEZE_LIFTED", payload)

def _gov_authority_growth_unlocked(new_state, payload: dict):
    dr = _ensure_drift_review_obj(new_state)
    if dr is None:
        return
    dr["authority_growth_locked"] = False
    _gov_record(new_state, "AUTHORITY_GROWTH_UNLOCKED", payload)

def _gov_d3_restored(new_state, payload: dict):
    dr = _ensure_drift_review_obj(new_state)
    if dr is None:
        return
    dr["d3_escalation_forbidden"] = False
    _gov_record(new_state, "D3_ESCALATION_RESTORED", payload)
# --- end Governance unlock wiring (v0.5) ---